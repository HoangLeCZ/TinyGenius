...
export type LanguageQuestion<T> = {
  id: string
  mediaUI: React.ReactNode
  prompt: {en:string, vi:string}
  answer: T
  choices: T[]
  word: {en:string, vi:string}
}
...
export default function LanguageEngine<T extends string | number>({
  ...
  // FIX 1: Load + Prime voices so Q1 works
  useEffect(() => {
    const loadVoices = () => {
  ...
  const speak = useCallback((text: string, ttsLang: 'en'|'vi' = lang) => {
    ...
    // If voices not ready, queue and retry
    if(!voicesReady) {
    ...
    if (ttsLang === 'vi') {
      ...
      // Best EN: Google US > Google UK > Microsoft > any en
  ...
  const nextQ = useCallback(async () => {
    ...
    if(qNum > total) {
      ...
      playSound('done')
      speak(lang==='en'?'Great job!':'Tuyệt vời!')
      setLoading(false)
      return
    }
    ...
    // FIX 3: Wait longer on first load
    setTimeout(() => {
      if(mode === 'listen') speak(newQ.word[lang], lang)
      else speak(newQ.prompt[lang], lang)
    }, voicesReady? 200 : 600)
  }, [qNum, total, generateQuestion, lang, speak, playSound, mode, voicesReady])
  ...
  const handleAnswer = (choice: T) => {
    ...
    if(correct) {
      ...
      playSound('wrong')
      speak(lang==='en'?'Try again':'Sai rồi', lang)
    }
