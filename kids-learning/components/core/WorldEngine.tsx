...
export default function WorldEngine<T extends string | number>({
  title, total, theme, backRoute, generateQuestion, getAnswerText, lang, speakQuestion = true, showFact = true
}: WorldEngineProps<T>){
  ...
  // FIX 2: Smart speak with retry + best voice
  ...
  const nextQ = useCallback(async () => {
    ...
    if(qNum > total) {
      ...
      playSound('done')
      speakText(lang==='en'?'You did it!':'Chúc mừng con!', lang)
      setLoading(false)
      return
    }
    ...
    // FIX 3: Wait longer for first question
    if(speakQuestion) {
      setTimeout(() => speakText(newQ.speakText?.[lang] || '', lang), voicesReady? 200 : 600)
    }
  }, [qNum, total, generateQuestion, lang, startTime, speakText, speakQuestion, playSound, voicesReady])
  ...
  const handleAnswer = (choice: T) => {
    ...
    if(correct) {
      setScore(s => s + 1)
      playSound('correct')
      speakText(lang==='en'?'Correct!':'Giỏi lắm!', lang)
    } else {
      playSound('wrong')
      speakText(lang==='en'?'Oops':'Chưa đúng rồi', lang)
    }
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 ${baloo.className} ${themeBg}`}>
      ...
      <div key={q?.id}>{q?.questionUI}</div> {/* key to force remount */}
      {showFact && q?.fact && (
